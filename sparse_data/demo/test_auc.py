...
train_auc, auc_op = sess.run([auc, auc_op])

print(train_auc, auc_op)
...
acc, acc_op = tf.metrics.auc(labels=x, predictions=y)
...
v, k = sess.run([acc, acc_op], feed_dict={x: [0, 1, 1, 1, 1],
                               y: [0, 1, 1, 1, 1]})
print(k)
